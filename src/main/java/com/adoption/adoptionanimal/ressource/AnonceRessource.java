...
public class AnonceRessource {
    ...
    public List<Anonce> getAll(){return anonceRepository.findAll();}
    ...
    public ResponseEntity<Anonce> createApplication(@RequestBody Anonce anonce) {
       ...
       /* Optional<Anonce> applicationdata = anonceRepository.findById(anonce.getAnimal_ID());
        if(!applicationdata.isPresent()){
            try {*/
                Anonce _application = anonceRepository.save(new Anonce(anonce.getAnimal_ID(),
                        anonce.getEmail_annonceur(),
                        anonce.getTelephone_annonceur(),
                        anonce.getAnimal_NOM(),anonce.getAnimal_RACE(),
                        anonce.getAnimal_AGE(),anonce.getVaccin(),
                        anonce.getAnimal_genre(),anonce.getDESCRIPTION(),
                        anonce.getDATE_ANONCE()));
    ...
    @DeleteMapping("delete/anonce/{id}")
    public String delete(@PathVariable Long id) {
        Optional<Anonce> anonce = anonceRepository.findById(id);
        anonceRepository.delete(anonce.get());
        return "Record deleted";
    }

    @PutMapping("/update")
    public Anonce update(@RequestBody Anonce anonceObj) {
        return anonceRepository.save(anonceObj);
    }
